<script>
  ...
  $: {
    const decodedBrand = decodeURIComponent($page.params.brand);
    if (decodedBrand !== brandName) {
  ...
  async function loadBrandData() {
    ...
    error = null;
    try {
      const response = await fetch(`http://localhost:8000/brand/${encodeURIComponent(brandName)}`);
      if (!response.ok) throw new Error('Бренд не найден');
      
      brandData = await response.json();
      filteredDevices = [...brandData.devices];

      if (brandData.devices.length > 0) {
        const prices = brandData.devices.map(d => d.Цена || 0);
        minPrice = Math.min(...prices);
        maxPrice = Math.max(...prices);
        realMaxPrice = maxPrice;
        
        const reviews = brandData.devices.map(d => d.Всего_отзывов || 0);
  ...
  function applyFilters() {
    if (!brandData?.devices) return;
    
    let filtered = [...brandData.devices];
...
<div class="brand-page">
  {#if loading}
    <div class="loading">
      ...
      <p>Загрузка данных о бренде...</p>
    </div>
  {:else if error}
    <div class="error">
      <h2>Ошибка</h2>
      <p>{error}</p>
      <button on:click={loadBrandData}>Попробовать снова</button>
    </div>
  {:else if brandData}
    <div class="brand-header">
      ...
      <div class="brand-stats">
        <div class="stat-card">
          <div class="stat-value">{formatNumber(brandData.device_count)}</div>
          <div class="stat-label">Товаров</div>
        </div>
        
        <div class="stat-card">
          <div class="stat-value">{brandData.avg_rating.toFixed(1)}</div>
          <div class="stat-label">Средний рейтинг</div>
        </div>
        
        <div class="stat-card">
          <div class="stat-value">{formatPrice(brandData.avg_price)}</div>
          <div class="stat-label">Средняя цена</div>
        </div>
        
        <div class="stat-card">
          <div class="stat-value">{formatNumber(brandData.total_reviews)}</div>
          <div class="stat-label">Отзывов</div>
        </div>
        
        <div class="stat-card">
          <div class="stat-value">{brandData.share_percent.toFixed(1)}%</div>
      ...
      <div class="products-section">
        <div class="search-container">
          ...
          <div class="results-info">
            Найдено товаров: <strong>{filteredDevices.length}</strong> из {brandData.device_count}
          ...
          {#each filteredDevices as device}
            <div class="product-card" on:click={() => goToProduct(device.id)}>
              <div class="product-image">
                <div class="image-placeholder">{brandData.brand.charAt(0)}</div>
              </div>
  ...
  {:else}
    <div class="empty-state">
      ...
      <button on:click={loadBrandData}>Загрузить данные</button>
    </div>
  {/if}
</div>
